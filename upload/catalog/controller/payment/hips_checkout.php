...
class ControllerPaymentHipsCheckout extends Controller
{
    ...
    public function index()
    {
        ...
        if ($this->request->get['route'] == "payment/hips_checkout") {
            ...
            $this->data['heading_title'] = $this->language->get('text_checkout_title');
            
            $this->data['text_empty']   = $this->language->get('text_empty');
            $this->data['section_left'] = $this->config->get('hips_mode_bar');
            
            if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/hips_checkout.tpl')) {
                $this->template = $this->config->get('config_template') . '/template/payment/hips_checkout.tpl';
            } else {
                $this->template = 'default/template/payment/hips_checkout.tpl';
            }
            
            $this->children = array(
                'payment/hips_checkout/left',
                'common/column_right',
                'common/content_top',
                'common/content_bottom',
                'common/footer',
                'common/header',
                'payment/hips_checkout/main'
            );
            ...
            $this->template = 'default/template/payment/hipsform.tpl';
            $this->response->setOutput($this->render());
        }
    }
    ...
    public function main()
    {
        ...
        $this->data['heading_title'] = $this->language->get('text_complete_title');
        ...
        list($totals, $total, $taxes) = $this->model_payment_hips_checkout->getTotals();
        ...
        if (!$redirect) {
            ...
            if (isset($this->session->data['hips_checkout_currency']) && $this->session->data['hips_checkout_currency'] != $this->session->data['currency']) {
                $this->model_payment_hips_checkout->log('Currency changed, unsetting kc order id');
                unset($this->session->data['hips_checkout_order_id']);
        ...
        $this->data['html_snippet'] = $html_snippet;
        
        $this->template = 'default/template/payment/hips_checkout_main.tpl';
        $this->response->setOutput($this->render());
                    ...
                    if ($order_info) {
                        ...
                        if ($order) {
                            ...
                            if ($order_status_id) {
                                ...
                                $this->model_checkout_order->confirm($hips_checkout_order['order_id'], $order_status_id);
                            }
                            ...
                            if (isset($jresult['status']) && $jresult['status'] != "") {
                                ...
                                $this->model_checkout_order->update($hips_checkout_order['order_id'], $this->config->get('hips_order_status_id'), $message, false);
    ...
    public function left()
    {
        ...
        $this->data['heading_title'] = $this->language->get('text_review_title');
        ...
        $this->template = 'default/template/payment/hips_checkout_left.tpl';
        $this->response->setOutput($this->render());
    ...
    private function createOrder()
    {
        ...
        if ($this->cart->hasShipping()) {
            ...
            $order_data['shipping_company_id']     = $this->session->data['shipping_address']['company_id'];
            $order_data['shipping_tax_id']         = $this->session->data['shipping_address']['tax_id'];
    ...
    public function send()
    {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            ...
            if (isset($jresult['status']) && $jresult['status'] != "") {
                ...
                $this->model_checkout_order->confirm($this->session->data['order_id'], $this->config->get('hips_order_status_id'));
                $this->model_checkout_order->update($this->session->data['order_id'], $this->config->get('hips_order_status_id'), $message, false);
